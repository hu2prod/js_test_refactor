...
void _Report_get_country_impl(
  ...
  ret     = (char*)(void*)line->country.c_str();
  ret_len = line->country.size();
}
...
napi_value Report_get_country_sync(napi_env env, napi_callback_info info) {
  ...
  char* ret = nullptr;
  size_t ret_len;
  _Report_get_country_impl(err, _this, idx, ret, ret_len);
  if (err) {
...
//   async
////////////////////////////////////////////////////////////////////////////////////////////////////
struct Worker_ctx_Report_get_country {
  Report* _this;
  u64 idx;
  char* ret;
  size_t ret_len;
...
void _worker_ctx_Report_get_country_clear(napi_env env, struct Worker_ctx_Report_get_country* worker_ctx) {
  if (worker_ctx->err) {
...
void _execute_Report_get_country(napi_env env, void* _data) {
  struct Worker_ctx_Report_get_country* worker_ctx = (struct Worker_ctx_Report_get_country*)_data;
  _Report_get_country_impl(worker_ctx->err, worker_ctx->_this, worker_ctx->idx, worker_ctx->ret, worker_ctx->ret_len);
}

void _complete_Report_get_country(napi_env env, napi_status execute_status, void* _data) {
  napi_status status;
  struct Worker_ctx_Report_get_country* worker_ctx = (struct Worker_ctx_Report_get_country*)_data;
  ...
  if (status != napi_ok) {
    ...
    _worker_ctx_Report_get_country_clear(env, worker_ctx);
    return;
  }
  ...
  status = napi_create_string_utf8(env, worker_ctx->ret, worker_ctx->ret_len, &call_argv[1]);
  if (status != napi_ok) {
    fprintf(stderr, "status = %d\n", status);
...
napi_value Report_get_country(napi_env env, napi_callback_info info) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Worker_ctx_Report_get_country* worker_ctx = new Worker_ctx_Report_get_country;
  worker_ctx->err = nullptr;
  ...
  status = napi_create_async_work(
    ...
    nullptr,
    async_resource_name,
    _execute_Report_get_country,
    _complete_Report_get_country,
    (void*)worker_ctx,
    &worker_ctx->work
  );
  if (status != napi_ok) {
