...
napi_value Report_set_city_sync(napi_env env, napi_callback_info info) {
  ...
  _Report_set_city_impl(err, _this, idx, val, val_len);
  if (err) {
...
//   async
////////////////////////////////////////////////////////////////////////////////////////////////////
struct Worker_ctx_Report_set_city {
  Report* _this;
  u64 idx;
  char* val;
  size_t val_len;
...
void _worker_ctx_Report_set_city_clear(napi_env env, struct Worker_ctx_Report_set_city* worker_ctx) {
  if (worker_ctx->err) {
...
void _execute_Report_set_city(napi_env env, void* _data) {
  struct Worker_ctx_Report_set_city* worker_ctx = (struct Worker_ctx_Report_set_city*)_data;
  _Report_set_city_impl(worker_ctx->err, worker_ctx->_this, worker_ctx->idx, worker_ctx->val, worker_ctx->val_len);
}

void _complete_Report_set_city(napi_env env, napi_status execute_status, void* _data) {
  napi_status status;
  struct Worker_ctx_Report_set_city* worker_ctx = (struct Worker_ctx_Report_set_city*)_data;
  ...
  if (status != napi_ok) {
    ...
    _worker_ctx_Report_set_city_clear(env, worker_ctx);
    return;
  }
...
napi_value Report_set_city(napi_env env, napi_callback_info info) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Worker_ctx_Report_set_city* worker_ctx = new Worker_ctx_Report_set_city;
  worker_ctx->err = nullptr;
  ...
  status = napi_create_async_work(
    ...
    nullptr,
    async_resource_name,
    _execute_Report_set_city,
    _complete_Report_set_city,
    (void*)worker_ctx,
    &worker_ctx->work
  );
  if (status != napi_ok) {
