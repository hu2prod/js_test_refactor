...
void _Report_pretty_print_impl(
  ...
  for (const auto& row : _this->line_list) {
    std::cout
      << std::left << std::setw(18) << row.city
      << std::right << std::setw(10) << row.population
      << std::right << std::setw(8) << row.area
      << std::right << std::setw(8) << row.density
      << std::right << std::setw(18) << row.country
      << std::right << std::setw(6) << row.density_percent
      << std::endl;
  }
}
...
napi_value Report_pretty_print_sync(napi_env env, napi_callback_info info) {
  ...
  _Report_pretty_print_impl(err, _this);
  if (err) {
...
//   async
////////////////////////////////////////////////////////////////////////////////////////////////////
struct Worker_ctx_Report_pretty_print {
  Report* _this;
...
void _worker_ctx_Report_pretty_print_clear(napi_env env, struct Worker_ctx_Report_pretty_print* worker_ctx) {
  if (worker_ctx->err) {
...
void _execute_Report_pretty_print(napi_env env, void* _data) {
  struct Worker_ctx_Report_pretty_print* worker_ctx = (struct Worker_ctx_Report_pretty_print*)_data;
  _Report_pretty_print_impl(worker_ctx->err, worker_ctx->_this);
}

void _complete_Report_pretty_print(napi_env env, napi_status execute_status, void* _data) {
  napi_status status;
  struct Worker_ctx_Report_pretty_print* worker_ctx = (struct Worker_ctx_Report_pretty_print*)_data;
  ...
  if (status != napi_ok) {
    ...
    _worker_ctx_Report_pretty_print_clear(env, worker_ctx);
    return;
  }
...
napi_value Report_pretty_print(napi_env env, napi_callback_info info) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Worker_ctx_Report_pretty_print* worker_ctx = new Worker_ctx_Report_pretty_print;
  worker_ctx->err = nullptr;
  ...
  status = napi_create_async_work(
    ...
    nullptr,
    async_resource_name,
    _execute_Report_pretty_print,
    _complete_Report_pretty_print,
    (void*)worker_ctx,
    &worker_ctx->work
  );
  if (status != napi_ok) {
