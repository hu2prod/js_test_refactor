...
napi_value Report_set_area_sync(napi_env env, napi_callback_info info) {
  ...
  _Report_set_area_impl(err, _this, idx, val);
  if (err) {
...
//   async
////////////////////////////////////////////////////////////////////////////////////////////////////
struct Worker_ctx_Report_set_area {
  Report* _this;
  u64 idx;
  i64 val;
...
void _worker_ctx_Report_set_area_clear(napi_env env, struct Worker_ctx_Report_set_area* worker_ctx) {
  if (worker_ctx->err) {
...
void _execute_Report_set_area(napi_env env, void* _data) {
  struct Worker_ctx_Report_set_area* worker_ctx = (struct Worker_ctx_Report_set_area*)_data;
  _Report_set_area_impl(worker_ctx->err, worker_ctx->_this, worker_ctx->idx, worker_ctx->val);
}

void _complete_Report_set_area(napi_env env, napi_status execute_status, void* _data) {
  napi_status status;
  struct Worker_ctx_Report_set_area* worker_ctx = (struct Worker_ctx_Report_set_area*)_data;
  ...
  if (status != napi_ok) {
    ...
    _worker_ctx_Report_set_area_clear(env, worker_ctx);
    return;
  }
...
napi_value Report_set_area(napi_env env, napi_callback_info info) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  Worker_ctx_Report_set_area* worker_ctx = new Worker_ctx_Report_set_area;
  worker_ctx->err = nullptr;
  ...
  status = napi_create_async_work(
    ...
    nullptr,
    async_resource_name,
    _execute_Report_set_area,
    _complete_Report_set_area,
    (void*)worker_ctx,
    &worker_ctx->work
  );
  if (status != napi_ok) {
