...
// NOTE. maybe missing 'napi_value __fn;'
...
#define FN_EXPORT_MANUAL(NAME, DST_NAME)                                                    \
status = napi_create_function(env, nullptr, 0, NAME, nullptr, &__fn);                       \
if (status != napi_ok) {                                                                    \
  napi_throw_error(env, nullptr, "FN_EXPORT !napi_create_function NAME=" TOSTRING(NAME));   \
  return nullptr;                                                                           \
                                                                                            ...
                                                                                            \
status = napi_set_named_property(env, exports, TOSTRING(DST_NAME), __fn);                   \
if (status != napi_ok) {                                                                    \
  napi_throw_error(env, nullptr, "FN_EXPORT !napi_set_named_property NAME=" TOSTRING(NAME));\
  return nullptr;                                                                           \
}
...
#define CLASS_METHOD_MANUAL(CLASS_NAME, METHOD_NAME, DST_METHOD_NAME)       \
PPCAT_NX(CLASS_NAME,_prop_list).push_back({ TOSTRING(DST_METHOD_NAME), nullptr, PPCAT_NX3(CLASS_NAME, _, METHOD_NAME), nullptr, nullptr, nullptr, napi_enumerable, nullptr });
